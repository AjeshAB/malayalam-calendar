 ...
 *  2026-08-10: only Thiruvathira (Nazhika 15.43). Punartham continues past next sunrise → next day. */
{
  ...
  assert.strictEqual(p.nakshatramDetails[0].endNazhika, 15.43);
}

/** 2026-10-16: Thrikketta ends after sunrise (Nazhika 1.31). Checkpoint Moolam spans
 *  past next sunrise — do not list it; it belongs to the next day. */
{
  ...
  assert.strictEqual(p.nakshatramDetails[0].endNazhika, 1.31);
  assert.strictEqual(
...
/** 2026-10-30: day's star is Makiryam (Nazhika 6.93). Thiruvathira is between checkpoints
 *  but must not be compounded into the day name — it ends the next day. */
{
  ...
  assert.strictEqual(p.nakshatramDetails[0].endNazhika, 6.93);
}

/** 2026-01-28: only Karthika ends (Nazhika 6.5). Checkpoint is Rohini but day name is Karthika. */
{
  const p = getPanchangam('2026-01-28');
  assert.strictEqual(p.nakshathram.en, 'Karthika');
  assert.strictEqual(p.nakshathram.ml, 'കാർത്തിക');
  assert.strictEqual(p.nakshatramDetails.length, 1);
  assert.strictEqual(p.nakshatramDetails[0].nakshatram.en, 'Karthika');
  assert.strictEqual(p.nakshatramDetails[0].endNazhika, 6.5);
}
